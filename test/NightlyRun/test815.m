%Test Name: SSA2dCDCalvingButtressingWaterDepth
md=triangle(model(),'../Exp/Square.exp',50000);
md=setmask(md,'','');
md=parameterize(md,'../Par/ValleyGlacierShelf.par');
md=setflowequation(md,'SSA','all');
md.timestepping.time_step=0.1;
md.timestepping.final_time=2;
md.cluster=generic('name',oshostname(),'np',3);

%Transient;
md.transient.isstressbalance=1;
md.transient.ismovingfront=1;
md.transient.ismasstransport=1;
md.transient.issmb=1;
md.transient.isthermal=0;
md.transient.isgroundingline=1;

md.calving=calvingcrevassedepth();
md.calving.crevasse_opening_stress=2;
md.calving.surface_hydrology_type=1;
md.calving.hurst=0.41*ones(md.mesh.numberofvertices,1);
md.calving.sigma = 14.3*ones(md.mesh.numberofvertices,1);
md.calving.melt_supply = 50*ones(md.mesh.numberofvertices+1,md.timestepping.final_time/md.timestepping.time_step +1);
md.calving.melt_supply(end,:) = [md.timestepping.start_time:md.timestepping.time_step:md.timestepping.final_time];
md.frontalforcings.meltingrate=zeros(md.mesh.numberofvertices,1);
md.levelset.spclevelset=NaN(md.mesh.numberofvertices,1);
md.levelset.reinit_frequency=1;
md.levelset.migration_max = 1e10;

md.transient.requested_outputs={'default', 'CrevasseDepth', 'BasalCrevasse', 'SurfaceCrevasse','WaterDepth'};
md=solve(md,'Transient');

%Fields and tolerances to track changes
field_names ={...
    'Vx1','Vy1','Vel1','Pressure1','Thickness1','Surface1','MaskIceLevelset1',...
    'CrevasseDepth1', 'BasalCrevasse1', 'SurfaceCrevasse1','WaterDepth1' ...
    'Vx2','Vy2','Vel2','Pressure2','Thickness2','Surface2','MaskIceLevelset2'...
    'CrevasseDepth2', 'BasalCrevasse2', 'SurfaceCrevasse2','WaterDepth2' ...
    'Vx10','Vy10','Vel10','Pressure10','Thickness10','Surface10','MaskIceLevelset10',...
    'CrevasseDepth10', 'BasalCrevasse10', 'SurfaceCrevasse10','WaterDepth10'...
    'Vx20','Vy20','Vel20','Pressure20','Thickness20','Surface20','MaskIceLevelset20',...
    'CrevasseDepth20', 'BasalCrevasse20', 'SurfaceCrevasse20','WaterDepth20'};
field_tolerances={...
    1e-8,1e-8,1e-8,1e-9,1e-9,1e-9,3e-9,1e-9,1e-9,1e-9,1e-8...
    1e-8,1e-8,1e-8,1e-9,1e-9,1e-9,3e-9,1e-9,1e-9,1e-9,1e-8...
    1e-8,1e-8,1e-8,1e-9,1e-9,1e-9,3e-9,1e-9,1e-9,1e-9,1e-8...
    1e-8,1e-8,1e-8,1e-9,1e-9,1e-9,3e-9,1e-9,1e-9,1e-9,1e-8};
field_values={...
    md.results.TransientSolution(1).Vx,...
    md.results.TransientSolution(1).Vy,...
    md.results.TransientSolution(1).Vel,...
    md.results.TransientSolution(1).Pressure,...
    md.results.TransientSolution(1).Thickness,...
    md.results.TransientSolution(1).Surface,...
    md.results.TransientSolution(1).MaskIceLevelset,...
    md.results.TransientSolution(1).CrevasseDepth,...
    md.results.TransientSolution(1).BasalCrevasse,...
    md.results.TransientSolution(1).SurfaceCrevasse,...
    md.results.TransientSolution(1).WaterDepth,...
    md.results.TransientSolution(2).Vx,...
    md.results.TransientSolution(2).Vy,...
    md.results.TransientSolution(2).Vel,...
    md.results.TransientSolution(2).Pressure,...
    md.results.TransientSolution(2).Thickness,...
    md.results.TransientSolution(2).Surface,...
    md.results.TransientSolution(2).MaskIceLevelset,...
    md.results.TransientSolution(2).CrevasseDepth,...
    md.results.TransientSolution(2).BasalCrevasse,...
    md.results.TransientSolution(2).SurfaceCrevasse,...
    md.results.TransientSolution(2).WaterDepth,...
    md.results.TransientSolution(10).Vx,...
    md.results.TransientSolution(10).Vy,...
    md.results.TransientSolution(10).Vel,...
    md.results.TransientSolution(10).Pressure,...
    md.results.TransientSolution(10).Thickness,...
    md.results.TransientSolution(10).Surface,...
    md.results.TransientSolution(10).MaskIceLevelset,...
    md.results.TransientSolution(10).CrevasseDepth,...
    md.results.TransientSolution(10).BasalCrevasse,...
    md.results.TransientSolution(10).SurfaceCrevasse,...
    md.results.TransientSolution(10).WaterDepth,...
    md.results.TransientSolution(20).Vx,...
    md.results.TransientSolution(20).Vy,...
    md.results.TransientSolution(20).Vel,...
    md.results.TransientSolution(20).Pressure,...
    md.results.TransientSolution(20).Thickness,...
    md.results.TransientSolution(20).Surface,...
    md.results.TransientSolution(20).MaskIceLevelset,...
    md.results.TransientSolution(20).CrevasseDepth,...
    md.results.TransientSolution(20).BasalCrevasse,...
    md.results.TransientSolution(20).SurfaceCrevasse,...
    md.results.TransientSolution(20).WaterDepth,...
    };